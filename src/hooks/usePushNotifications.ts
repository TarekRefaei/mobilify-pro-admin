...
export const usePushNotifications = () => {
  ...
  useEffect(() => {
    ...
    const initializeNotifications = async () => {
      try {
        ...
        unsubscribe = pushNotificationService.subscribeToNotifications(
          updatedNotifications => {
            setNotifications(updatedNotifications);

            // Calculate stats
            const sentNotifications = updatedNotifications.filter(
              n => n.status === 'sent'
            );
            ...
            const thisWeek = sentNotifications.filter(
              n => new Date(n.sentAt || n.createdAt) > weekAgo
            ).length;
  ...
  const getNotificationsByAudience = (audience: string) => {
    return notifications.filter(
      notification => notification.targetAudience === audience
    );
  };
